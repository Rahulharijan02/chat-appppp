...
from .templatetags.social_extras import liked_by, user_liked
...
class PluginSmokeTests(TestCase):
    ...
    def test_liked_helpers_honor_prefetched_flag(self):
        """Both filter and tag helpers should trust an annotated is_liked flag."""

        post = Post.objects.create(author=self.bob, body='Flag check', visibility='public')
        post.is_liked = True

        self.assertTrue(liked_by(post, self.alice))
        self.assertTrue(user_liked(post, self.alice))

        post.is_liked = False
        self.assertFalse(liked_by(post, self.alice))
        self.assertFalse(user_liked(post, self.alice))

    def test_user_liked_handles_anonymous_safely(self):
        """Anonymous users should never trigger like lookups or truthy values."""

        post = Post.objects.create(author=self.bob, body='Public check', visibility='public')
        anon_user = type('Anon', (), {'is_authenticated': False})()

        self.assertFalse(user_liked(post, anon_user))
